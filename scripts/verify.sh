...
target="${1:-all}"
case "$target" in base|rvn|rvn-vision|all) ;; *) printf 'Usage: %s [base|rvn|rvn-vision|all]\n' "$0" >&2; exit 2 ;; esac
...
if [[ "$target" = rvn-vision || "$target" = all ]]; then
  python3 "$RELEASE_ROOT/scripts/verify_rvn_vision.py" \
    --model-dir "$RVN_VISION_MODEL_DIR" \
    --source-lock "$RVN_VISION_SOURCE_LOCK"
fi
